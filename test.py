...
class MyNet:
	def __init__(self):
		self.input_pl = tf.placeholder(tf.float32, shape=[args.batch_size,imsize,imsize,3])
		self.label_pl = tf.placeholder(tf.int32, shape=[args.batch_size,imsize,imsize])
		...
		logits_reshaped = tf.reshape(self.logits_tf, (args.batch_size*imsize*imsize,2))
		labels_reshaped = tf.reshape(self.label_pl, [args.batch_size*imsize*imsize])
...
input_images = numpy.zeros((args.batch_size, imsize, imsize, 3), dtype=numpy.float32)
input_labels = numpy.zeros((args.batch_size, imsize, imsize), dtype=numpy.int32)
img_history = []
ap_gt = []
ap_dt = []
comp_time = []
...
#	print('Loss %.2f(%.2f+%.2f) tp:%d fp:%d fn:%d prc:%.2f rcl:%.2f'%(ls,pl,nl,vtp,vfp,vfn,prc,rcl))
	...
	viz_predicted[ybound:ybound+imscale, xbound:xbound+imscale] = predicted
	...
	num_gt = 0
	num_dt = 0
	min_cluster_size = 10 if args.dataset=='beach' or args.dataset=='shore' else 200
	...
	if viz:
		...
		plt.subplot(2,2,3)
		plt.imshow(dt_viz if zoomed_in else viz_predicted, cmap='gray')
		plt.subplot(2,2,4)
